...
echo "${certs}" | awk '/subject.*CN.*=.*MO Sub1 CA/,/END CERTIFICATE/' > "${script_dir}"/../config/certificates/mo_sub_ca1.pem
echo "${certs}" | awk '/subject.*CN.*=.*MO Sub2 CA/,/END CERTIFICATE/' > "${script_dir}"/../config/certificates/mo_sub_ca2.pem
